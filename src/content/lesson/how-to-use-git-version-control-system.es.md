...
Digamos que creas tres confirmaciones de esta manera (ejemplo en la siguiente imagen), tu repositorio se verá así:
...
:warning:
En nuestros repetitivos, ya tendrá una configuración de origen desde el repositorio del repetitivo. En esos casos, no debe eliminar el origen original, sino que necesitarás usar otro término (por ejemplo, `new` o `new-origin`, etc.) para agregar su nuevo control remoto.
